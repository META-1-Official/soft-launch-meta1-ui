...
class MarketsStore {
	...
	onSubscribeMarket(result) {
		...
		if (result.calls) {
			...
			result.calls.forEach((call) => {
				...
				try {
					let mcr = this[this.invertedCalls ? 'baseAsset' : 'quoteAsset'].getIn(
		...
		if (result.history && activeMarketName === subscribedMarketName) {
			...
			result.history.forEach((order) => {
				...
				if (
					!order.op.is_maker &&
					!(order.op.receives.amount == 0 || order.op.pays.amount == 0)
				) {
	...
	_getFeed() {
		...
		try {
			...
			if (
				...
				feedPriceRaw = feedPriceRaw.setIn(['base', 'asset_id'], backingAsset);
	...
	_groupedOrderBook(groupedOrdersBids = null, groupedOrdersAsks = null) {
		...
		if (groupedOrdersBids && groupedOrdersAsks) {
			if (__DEV__) console.time('Sum grouped orders ' + this.activeMarket);
			...
			if (__DEV__) console.timeEnd('Sum grouped orders ' + this.activeMarket);
		}
	}
	...
	_invertMarketStats(stats, market) {
		...
		return {
			invertedStats: {
				change: ((1 / (1 + parseFloat(stats.change) / 100) - 1) * 100).toFixed(
					2
				),
