...
import {ChainStore, ChainValidation, FetchChain} from 'meta1-vision-js';
...
class AccountStore extends BaseStore {
	...
	_checkReferrer() {
		...
		if (referralAccount) {
			...
			if (__DEV__) console.log('Stored Referral Account: ', referralAccount);
		}
	...
	loadDbData() {
		...
		return new Promise((resolve, reject) => {
			iDB
				...
				.then((data) => {
					this.state.linkedAccounts = Immutable.fromJS(data || []).toSet();
					...
					if (!!this.state.passwordLogin) {
						accountPromises = !!this.state.passwordAccount
							? [FetchChain('getAccount', this.state.passwordAccount)]
							: data
									.filter((a) => {
										if (a.chainId) {
						...
						accountPromises = !!this.state.passwordlessAccount
							? [FetchChain('getAccount', this.state.passwordlessAccount)]
							: data
									.filter((a) => {
										if (a.chainId) {
											return a.chainId === chainId;
						...
						.then((accounts) => {
							accounts.forEach((a) => {
								if (
									!!a &&
									this.isMyAccount(a) &&
									!this.state.myHiddenAccounts.has(a.get('name'))
								) {
							...
							if (this.state.myActiveAccounts !== immutableAccounts) {
								this.setState({
	...
	addAccountRefs() {
		...
		let myActiveAccounts = this.state.myActiveAccounts.withMutations(
			(accounts) => {
				account_refs.forEach((id) => {
					 ...
					 */
					const nameOnlyEntry = this.state.linkedAccounts.findKey((a) => {
						return a.get('name') === linkedEntry.name && !a.has('chainId');
					});
					if (!!nameOnlyEntry) {
						this.state.linkedAccounts =
							this.state.linkedAccounts.delete(nameOnlyEntry);
		...
		if (!!this.state.passwordlessLogin) {
			myActiveAccounts = Immutable.Set(
				!!this.state.passwordlessAccount ? [this.state.passwordlessAccount] : []
			);
		}
	...
	getMyAccounts() {
		 ...
		 */
		if (this.state.passwordLogin)
			return !!this.state.passwordAccount ? [this.state.passwordAccount] : [];
		if (this.state.passwordlessLogin)
