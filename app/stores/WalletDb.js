...
import {ChainStore, PrivateKey, key, Aes} from 'meta1-vision-js';
...
class WalletDb extends BaseStore {
	...
	process_transaction(tr, signer_pubkeys, broadcast, extra_keys = []) {
		...
		const passwordlessLogin =
			SettingsStore.getState().settings.get('passwordlessLogin');
			...
			.then(() => {
				...
				return Promise.all([
					...
					if (signer_pubkeys) {
						...
						// potential keys.
						let pubkeys =
							PrivateKeyStore.getPubkeys_having_PrivateKey(signer_pubkeys);
		...
		let walletCreateFct = (dictionary) => {
			return new Promise((resolve, reject) => {
				...
				let encrypted_brainkey =
					local_aes_private.encryptToHex(brainkey_plaintext);
	...
	generateNextKey(save = true) {
		...
		for (let i = sequence; i < loopMax; i++) {
			...
			let pubkey = this.generateNextKey_pubcache[i]
				...
				: (this.generateNextKey_pubcache[i] = private_key
						.toPublicKey()
						.toPublicKeyString());
			...
			worker.onmessage = (event) => {
				try {
					...
					for (let i = 0; i < private_key_objs.length; i++) {
						...
						let {import_account_names, public_key_string, private_plainhex} =
							private_key_obj;
						...
						if (!public_key_string) {
							...
							public_key_string.indexOf(ChainConfig.address_prefix) != 0
						)
							throw new Error(
					...
					try {
						...
						resolve(
							Promise.all([insertKeysPromise, addyIndexPromise]).then(() => {
