...
class SettingsStore {
	...
	_getChainId() {
		return (
			Apis.instance().chain_id || chainIds[process.env.CURRENT_NET]
		).substr(0, 8);
	}
	...
	onUpdateUnits() {
		...
		if (this.defaults.unit.indexOf(this.settings.get('unit')) === -1) {
			...
			this.settings = this.settings.set('fee_asset', this.defaults.unit[0]);
		}
	}
	...
	onDeleteChartLayout(name) {
		if (name) {
			const index = this.chartLayouts.findIndex((item) => item.name === name);
			if (index !== -1) {
				this.chartLayouts = this.chartLayouts.delete(index);
			}
			ss.set('chartLayouts', this.chartLayouts.toJS());
