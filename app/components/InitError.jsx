...
class InitError extends React.Component {
	...
	render() {
		...
		let selectOptions = uniqueNodes.map((entry) => {
			...
			let {location} = entry;
			if (!!location && typeof location === 'object' && 'translate' in location)
				...
				<div className="grid-container">
					<div className="grid-content no-overflow">
						...
						<Form layout="vertical">
							<Form.Item label={counterpart.translate('settings.apiServer')}>
								<Input.Group compact>
									<Select
							...
							<Form.Item label={counterpart.translate('app_init.ws_status')}>
								{this.props.rpc_connection_status === 'open' ? (
									...
									<span className="txtlabel warning">
										<Translate content={`app_init.not_connected`} />
									</span>
								)}
							</Form.Item>

							<Button type={'primary'} onClick={this.onReloadClick}>
...
export default connect(InitError, {
	...
	getProps() {
		return {
			...
			defaultConnection:
				SettingsStore.getState().defaultSettings.get('apiServer'),
		};
	},
});
