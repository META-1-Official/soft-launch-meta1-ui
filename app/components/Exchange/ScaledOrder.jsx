...
class ScaledOrderForm extends Component {
	...
	_getTotal() {
		...
		for (let i = 0; i < orderCount; i += 1) {
			total += Number((amountPerOrder * (priceLower + step * i)).toFixed(6));
		}
	...
	render() {
		...
		if (formValues.action === SCALED_ORDER_ACTION_TYPES.BUY) {
			columns = [
				...
				{
					title: counterpart.translate('scaled_orders.preview_table.price'),
					dataIndex: 'price',
					key: 'price',
		...
		return (
			<Form layout="vertical" hideRequiredMark={true}>
				<Row>
					<Col span={11}>
						<Form.Item
							label={counterpart.translate('scaled_orders.action.title')}
						>
							{actionRadio}
						</Form.Item>
					</Col>
				...
				<Row>
					<Col span={11}>
						<Form.Item label={counterpart.translate('scaled_orders.quantity')}>
							{quantityInput}
						</Form.Item>
					</Col>
					<Col span={11} offset={2}>
				...
				<Row>
					<Col span={11}>
						<Form.Item label={counterpart.translate('scaled_orders.fee')}>
							{feeInput}
						</Form.Item>
					</Col>
				...
				<Row>
					<Col span={11}>
						<Form.Item label={counterpart.translate('scaled_orders.total')}>
							{totalInput}
						</Form.Item>
					</Col>
				</Row>
				<Row>
					<Col span={11}>
						<Form.Item
							label={counterpart.translate('scaled_orders.distribution.title')}
						>
							{distributionRadio}
						</Form.Item>
					</Col>
...
class ScaledOrderModal extends Component {
	...
	prepareOrders(values) {
		...
		const sellAmount = (i) =>
			values.action === SCALED_ORDER_ACTION_TYPES.BUY
				? Number((amountPerOrder / (priceLower + step * i)).toFixed(6)) *
