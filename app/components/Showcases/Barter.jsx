...
export default class Barter extends Component {
	...
	_updateFee(fee_asset_id, account, asset_id, index, from, barter) {
		const {from_fee_asset_types, to_fee_asset_types} = this._getAvailableAssets(
			this.state
		);
		const fee_asset_types = from ? from_fee_asset_types : to_fee_asset_types;
		if (fee_asset_types.length === 1 && fee_asset_types[0] !== fee_asset_id) {
			fee_asset_id = fee_asset_types[0];
		}
		if (!account) return null;

		let memo_state = this.state.memo[from ? 'from_barter' : 'to_barter'][index];
	...
	onSubmit(e) {
		...
		let escrowMemo =
			this.state.memo['escrow'][0] && this.state.memo['escrow'][0].message;
	...
	renderBalanceWarnings() {
		...
		let peer1Component = isPeer1Warning ? (
			<div style={{maxWidth: '25rem'}}>
				{counterpart.translate('showcases.barter.balance_warning_tooltip', {
					peer: peer1Text,
				})}
				<br />
				{peer1.map((item) => {
					if (item.warning) {
						return (
							<React.Fragment>
								<br />
								<span style={{marginRight: '10px'}} key={item.assetId}>
									{' - ' +
			...
			<span className="barter-balance-warning">
				{isPeer1Warning && (
					<Popover
						...
						title={counterpart.translate('showcases.barter.balance_warning')}
					>
						<span style={{cursor: 'help'}}>
							<Alert
								...
								message={
									peer1Text +
									' ' +
									counterpart.translate('showcases.barter.balance_warning')
								}
								type="warning"
								showIcon
							/>
	...
	render() {
		...
		let isEscrowNotValid = this.state.showEscrow && !this.state.escrow_account;
			...
			return (
				<div key={amount_index++}>
					<div style={{position: 'relative'}}>
						{!isMemoShown && (
							<Tooltip
								title={counterpart.translate('tooltip.add_memo_field')}
								placement="topLeft"
							>
								<Button
					...
					<div>
						...
						<div style={{paddingTop: '10px', paddingBottom: '10px'}}>
							<Button
								...
								disabled={
									!from_account ||
									!this.state.from_barter[this.state.from_barter.length - 1]
										.from_amount
								}
							>
								+ Add asset
					...
					<div>
						...
						<div style={{paddingTop: '10px', paddingBottom: '10px'}}>
							<Button
								...
								disabled={
									!to_account ||
									!this.state.to_barter[this.state.to_barter.length - 1]
										.to_amount
								}
							>
								+ Add asset
		...
		if (isSubmitNotValid) {
			if (!from_account) {
				...
				(from_account.get('id') == this.state.escrow_account.get('id') ||
		...
		let offers = (
			<Card style={{borderRadius: '10px'}}>
				{!isSubmitNotValid && (
					<div className="left-label" style={{fontSize: '1rem'}}>
						...
						{this.state.showEscrow &&
							...
							counterpart.translate('showcases.barter.escrow_as_witness', {
								escrow: this.state.escrow_account.get('name'),
							})}
						{this.state.showEscrow &&
							this.state.send_to_escrow &&
							counterpart.translate('showcases.barter.escrow_as_custodian', {
								escrow: this.state.escrow_account.get('name'),
							})}
					</div>
				)}
				...
				<Tooltip
					title={counterpart.translate('showcases.barter.add_escrow_tooltip')}
					placement="topRight"
				>
					<Button
				...
				{from_barter.length === 500 && to_barter.length === 500 ? ( // deactivate for now
					<div className="amount-selector" style={this.props.style}>
						...
						<div className="inline-label input-wrapper">
							<Input disabled={false} type="text" value={explictPrice()} />

							<div className="form-label select floating-dropdown">
								<div className="dropdown-wrapper inactive">
									<div>{`${assetFromSymbol}/${assetToSymbol}`}</div>
								</div>
							</div>
						</div>
					</div>
		...
		let totalFeeFrom = (
			<Card style={{borderRadius: '10px'}}>
				...
				<Tooltip
					title={counterpart.translate('showcases.barter.proposal_fee_tooltip')}
				>
					<div>
						{/*needed to render tooltip properly*/}
						<AmountSelector
				...
				<Tooltip
					title={counterpart.translate('showcases.barter.total_fees_tooltip')}
				>
		...
		let intro = (
			...
			>
				<Tooltip
					title={counterpart.translate('showcases.barter.new_barter_tooltip')}
					placement="bottom"
				>
