			...
			finishedCard = (
				<Card>
					<div className={'center-content'}>
						<Translate content={'showcases.borrow.choose'} component={'h4'} />
					</div>
					<div
						style={{
							display: 'flex',
					...
					>
						<div>
							...
							/>
							<Tooltip
								title={counterpart.translate('showcases.borrow.borrow_tooltip')}
								placement="bottom"
							>
								<Button
									type="primary"
									...
									disabled={
										this.state.selectedAsset !== null && accountLoaded
					...
					>
						...
						{finishedCard == null && (
							<Card onKeyDown={this.onKeyDown.bind(this)}>
								{!!steps[current].icon && <Icon name="steps[current].icon" />}
								<Translate
									component="h2"
									content={
								...
								<Translate
									component="p"
									content={
										'showcases.borrow.steps_' + steps[current].key + '.text'
									}
								/>
										...
										{legend.map((content, index) => {
											return (
												<p key={'borrow_subp_' + index}>
													<strong>{content[0]}</strong>: {content[1]}
												</p>
											);
										})}
									</React.Fragment>
					...
					<div className="steps-action">
						{current < steps.length && (
							...
							>
								...
								>
									{current == 0 && (
										<Translate content={'showcases.borrow.get_started'} />
									)}
									{current > 0 && current < steps.length - 1 && (
										<Translate content={'showcases.borrow.next'} />
									)}
