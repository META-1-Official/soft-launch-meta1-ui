...
class Htlc extends Component {
	...
	async _update() {
		...
		if (
			...
			let htlc_list = await HtlcActions.getHTLCs(currentAccount.get('id'));
	...
	render() {
		const {isModalVisible, htlc_list, operationData, filterString} = this.state;
		let currentAccount = this.props.currentAccount;
		...
		const columns = [
			...
			},
			{
				...
				render: (text, record) => {
					return counterpart.localize(new Date(utils.makeISODateString(text)), {
						type: 'date',
						format: 'full',
					});
				},
					...
					if (record.type) {
						return record.type === 'payer' ? (
							<span>
								...
								>
									{counterpart.translate('showcases.htlc.extend')}
								</Button>
							</span>
						) : (
							<span
							...
							>
								<Button>
									{counterpart.translate('showcases.htlc.redeem')}
								</Button>
							</span>
						);
					} else {
