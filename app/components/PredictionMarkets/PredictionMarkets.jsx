...
class PredictionMarkets extends Component {
	constructor(props) {
		...
		this.onCreatePredictionMarketModalOpen =
			this.onCreatePredictionMarketModalOpen.bind(this);
		this.onCreatePredictionMarketModalClose =
			this.onCreatePredictionMarketModalClose.bind(this);
		...
		this.handleUnknownHousesToggleChange =
			this.handleUnknownHousesToggleChange.bind(this);
		this.handleInvalidAssetsChecked =
		...
		if (fetchedAssets) {
			...
			fetchedAssets.forEach((item) => {
				if (!item.forPredictions) {
					item.forPredictions = {
						description: assetUtils.parseDescription(item.options.description),
						flagBooleans: assetUtils.getFlagBooleans(item.options.flags, true),
					};
				}
			});
	...
	_updatePredictionMarketsList(fetchedAssets = null) {
		...
		const assets = fetchedAssets.filter((asset) => {
			if (asset.bitasset_data && asset.bitasset_data.is_prediction_market) {
				if (
	...
	_updateOpinionsList(fetchedOpinions) {
		...
		fetchedOpinions.forEach((order, order_id) => {
			const opinion =
				order.market_base === order.sell_price.base.asset_id ? 'no' : 'yes';
			const refPrice =
			...
			if (flagBooleans['charge_market_fee']) {
				fee = Math.min(
					...
					(amount.amount * selectedMarket.options.market_fee_percent) / 10000
				);
			}
	...
	handleInvalidAssetsChecked() {
		const isHideInvalidAssetsChecked = !this.state.isHideInvalidAssetsChecked;
		this.setState(
			{
			...
			<div>
				...
				>
					<div className="filter-block">
						...
						>
							<Radio value={'all'}>
								{counterpart.translate('prediction.overview.all')}
							</Radio>
							<Radio value={'open'}>
								{counterpart.translate('prediction.overview.open')}
							</Radio>
			...
			<div>
				...
				>
					<div className="filter-block">
						...
						>
							<Radio value={'all'}>
								{counterpart.translate('prediction.details.all')}
							</Radio>
							<Radio value={'yes'}>
								{counterpart.translate('prediction.details.proves_true')}
							</Radio>
							<Radio value={'no'}>
								{counterpart.translate('prediction.details.incorrect')}
							</Radio>
						</Radio.Group>
					</div>
					<span className="action-buttons">
		...
		return (
			...
			>
				...
				{!this.state.isFetchingFinished ? (
					<LoadingIndicator
						loadingText={counterpart.translate('prediction.overview.loading')}
					/>
				) : null}
			</div>
		);
