	...
	getHeader() {
		...
		return [
			...
			},
			{
				...
				align: 'left',
				sorter_inactive: (a, b) => {
					let a_name = ChainStore.getAccount(a.opinionator).get('name');
					let b_name = ChainStore.getAccount(b.opinionator).get('name');
						...
						>
							<span>
								{counterpart.translate(
									'prediction.details.' +
										(item == 'yes' ? 'proves_true' : 'incorrect')
								)}
							</span>
						</div>
					);
			...
			},
			{
				...
				render: (item) => {
					return (
						...
						>
							<FormattedAsset amount={item.amount} asset={item.asset_id} />
						</div>
					);
				},
			},
				...
				render: (item, row) => {
					return (
						...
						>
							<FormattedAsset amount={item.amount} asset={item.asset_id} />
							&nbsp;(
							{((row.commission.amount / row.premium.amount) * 100).toPrecision(
								3
							)}
							%)
						</div>
			...
			{
				title: counterpart.translate('prediction.details.potential_profit'),
			...
			},
			{
				...
				render: (dataItem) => {
					return (
						...
						>
							{currentAccountId && dataItem.opinionator === currentAccountId ? (
								<Button
									onClick={() => {
										this.props.onCancel(dataItem);
									}}
								>
									{counterpart.translate('prediction.details.cancel')}
								</Button>
							) : (
								<React.Fragment>
									<span>
										...
										>
											{counterpart.translate('prediction.details.oppose')}
										</Button>
									</span>
								</React.Fragment>
							)}
