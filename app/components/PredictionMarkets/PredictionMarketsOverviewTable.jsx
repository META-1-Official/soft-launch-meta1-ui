...
class PredictionMarketsOverviewTable extends Component {
	...
	getHeader() {
		...
		return [
			...
			{
				title: counterpart.translate('prediction.overview.market_confidence'),
					...
					if (ticker) {
						if (
							...
							ticker.quote_volume = utils.convert_typed_to_satoshi(
								...
								ChainStore.getAsset(
									row.asset[1].bitasset_data.options.short_backing_asset
								)
							);
						}
						if (
						...
						return (
							<span>
								...
								<FormattedAsset
									...
									asset={row.asset[1].bitasset_data.options.short_backing_asset}
								/>
								&nbsp;
								{/*({ticker.percent_change})&nbsp;*/}
							</span>
				...
				render: (item, row) => {
					...
					if (ticker) {
						if (
							...
							ticker.latest =
								(parseFloat(ticker.latest) * 100).toPrecision(3) + '%';
						}
						if (
							!ticker.highest_bid ||
							...
							ticker.highest_bid =
								(parseFloat(ticker.highest_bid) * 100).toPrecision(3) + '%';
						}
						if (
							!ticker.lowest_ask ||
							ticker.lowest_ask === '0' ||
							...
							ticker.lowest_ask =
								(parseFloat(ticker.lowest_ask) * 100).toPrecision(3) + '%';
						}
						return ticker.latest !== '-' ? (
							<React.Fragment>
								...
								<span className="supsub">
									<sup className="superscript">{ticker.highest_bid}</sup>
									<sub className="subscript">{ticker.lowest_ask}</sub>
								</span>
								&nbsp;&nbsp;&nbsp;
							</React.Fragment>
						) : (
					...
					return (
						...
						>
							{isOwnedByCurrent(dataItem.issuer) ? (
								...
								>
									{counterpart.translate('prediction.overview.resolve')}
								</Button>
							) : (
								<div
									style={{
	...
	componentDidUpdate(prevProps) {
		if (
			prevProps.predictionMarkets.length !== this.props.predictionMarkets.length
		) {
			this.props.predictionMarkets.forEach((market) => {
				if (!(market.asset[1].id in Object.keys(this.tickersLoaded))) {
					...
					MarketsActions.getTicker(
						market.asset[1].bitasset_data.options.short_backing_asset,
						market.asset[1].id
					).then((result) => {
