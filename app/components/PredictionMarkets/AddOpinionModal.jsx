...
import {Modal, Input, Form, Radio, notification, Button} from 'antd';
...
export default class AddOpinionModal extends Modal {
	...
	_createOrder() {
		...
		if (type === 'shortAndSell') {
			...
			MarketsActions.createPredictionShort(sell, collateral).then((result) => {
				this.setState({inProgress: false});
				if (result.error) {
					if (result.error.message !== 'wallet locked')
						notification.error({
	...
	render() {
		const {showWarning, newOpinionParameters, wrongProbability} = this.state;
				...
				<div className="prediction-markets--add-prediction-offer">
					<Form className="full-width" layout="vertical">
						...
						<Form.Item>
							<span
								className={
									(!newOpinionParameters.probability && showWarning) ||
									wrongProbability
										? 'has-error'
										: ''
								}
