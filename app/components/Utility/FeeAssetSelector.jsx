...
class FeeAssetSelector extends React.Component {
	...
	shouldComponentUpdate(np, ns) {
		...
		const transactionChanged =
			JSON.stringify(np.transaction) !== JSON.stringify(this.props.transaction);
		if (ns.assets) {
	...
	_getAsset() {
		...
		return feeAsset ? feeAsset : assets && assets.length > 0 ? assets[0] : null;
	}
				...
				>
					<Input.Group compact>
						...
						<AssetSelect
							...
							onDropdownVisibleChange={this._syncAvailableAssets.bind(this)}
							style={{width: '130px'}}
							selectStyle={{width: '100%'}}
							value={currentAsset.get('symbol')}
							assets={
								canChangeFeeParams ? Immutable.List(selectableAssets) : []
							}
