 ...
 *             value: "1.2.1"|{amount: 10, asset_id: process.env.META1_ASSET_ID}|"1.3.1"|{base: {amount: 1, asset_id: process.env.META1_ASSET_ID}, quote: {amount: 100, asset_id: "1.3.20"}}},
			...
			if (splitText.indexOf(key.arg)) {
				...
				switch (key.type) {
					...
					case 'icon':
						...
						value = (
							<Icon className={key.className} name={key.value} title={title} />
						);
						break;
					...
					case 'change':
						if (key.value && Object.keys(key.value).length > 0) {
							...
							const warning = (active || owner || memo) && (
								<div>
									<Translate content={'proposal.permission_changes'} />
									{', '}
									<Translate
										style={{color: 'red'}}
							...
							const activeDiv = active && (
								<React.Fragment>
									<Translate content={'proposal.changes_to_active'} />
										...
										{active.keys.plus.length > 0 ||
											(active.accounts.plus.length > 0 && (
												<div>
													...
													{active.accounts.plus.map((_tmp) => (
														<span key={_tmp}>{this.linkToAccount(_tmp)}</span>
													))}
												</div>
											))}
										{active.keys.minus.length > 0 ||
											(active.accounts.minus.length > 0 && (
												<div>
													...
													{active.accounts.minus.map((_tmp) => (
														<span key={_tmp}>{this.linkToAccount(_tmp)}</span>
													))}
												</div>
											))}
							...
							const memoDiv = memo &&
								(memo.keys.plus.length > 0 || memo.keys.minus.length > 0) && (
									<div>
