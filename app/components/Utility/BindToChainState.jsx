	...
	class Wrapper extends React.Component {
		...
		_update(next_props = null) {
			...
			for (let key of this.chain_account_names) {
				...
				if (prop) {
					...
					if (
						...
						new_state[key] = name;
					else if (name && name !== this.state[key]) new_state[key] = name;
					++all_objects_counter;
			...
			for (let key of this.chain_assets) {
				...
				if (prop) {
					...
					if (
						...
						new_state[key] = new_obj;
					else if ((new_obj && new_obj !== this.state[key]) || new_obj === null)
						new_state[key] = new_obj;
			...
			for (let key in temp_new_state) {
				if (!utils.are_equal_shallow(temp_new_state[key], temp_state[key])) {
					stateChanged = true;
				} else {
					...
					if (this.hasErrored) {
						return (
							<span style={{color: 'red'}}>
								Error rendering component, please report (see browser console
								for details)
							</span>
						);
					} else if (typeof options !== 'undefined' && options.show_loader) {
						console.error(
							'Required prop ' +
								prop +
						...
						return (
							<React.Fragment>
								<LoadingIndicator />
								<span className="text-center">Component re-rendering ...</span>
							</React.Fragment>
						);
					} else {
