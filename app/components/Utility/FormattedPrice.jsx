...
class FormattedPrice extends React.Component {
	...
	goToMarket(e) {
		...
		this.props.history.push(
			`/market/${!inverted ? first.get('symbol') : second.get('symbol')}_${
				!inverted ? second.get('symbol') : first.get('symbol')
			}`
		);
	}
	...
	render() {
		...
		let symbols = hide_symbols ? (
			...
			>
				...
				>
					<AssetName name={quote.get('symbol')} noTip={!this.props.noTip} />
					/
					<AssetName name={base.get('symbol')} noTip={!this.props.noTip} />
				</span>
			</Tooltip>
		);
