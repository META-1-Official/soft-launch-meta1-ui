			...
			decompress(compressed_data, (result) => {
				...
				if (this._validateFormat(invoice)) {
					FetchChainObjects(ChainStore.getAsset, [invoice.currency]).then(
						(assets_array) => {
							this.setState({invoice, asset: assets_array[0]}, this.getTotal);
						}
					);
				} else {
					this.setState({
	...
	render() {
		if (this.state.error)
			return (
				<div>
					<br />
					<h4 className="has-error text-center">{this.state.error}</h4>
				</div>
			);
		...
		let items = invoice.line_items.map((i, index) => {
			...
			return (
				<Row key={'invoice_item_' + index}>
					...
					<Col span={5}>
						<FormattedAsset amount={amount} asset={asset} exact_amount={true} />
					</Col>
				</Row>
			);
		});
