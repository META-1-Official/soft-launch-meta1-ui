...
import {Button, Progress, Form, Input} from 'antd';
...
class PasswordInput extends Component {
	...
	handlePasswordChange(e) {
		this.setState(
			...
			() => {
				if (this.props.onChange) this.props.onChange(this.state.password);
	...
	calculatePasswordScore(password) {
		...
		let passwordScore = Math.floor(password.length / (passwordLength * 1.5));
	...
	render() {
		...
		const getPasswordHelp = () => {
			if (this.state.isPasswordInputActive || !this.state.password) return '';
		...
		return [
			<Form.Item
				label={
					this.props.label || counterpart.translate('wallet.enter_password')
				}
				key="password-field"
