...
import {Switch, Tooltip, Row, Col, Radio, Input, Button} from 'antd';
...
class AccountVoting extends React.Component {
	...
	shouldComponentUpdate(np, ns) {
		return (
			...
			ns.showCreateCommitteeModal !== this.state.showCreateCommitteeModal ||
	...
	_getVoteObjects(type = 'witnesses', vote_ids) {
		...
		if (!vote_ids) {
			...
			let active = this.props.globalObject
				.get(isWitness ? 'active_witnesses' : 'active_committee_members')
				.sort((a, b) => {
				...
				this.state[`all_${type}`] = current.concat(
					Immutable.List(
						vote_objs
							...
							.map((a) =>
								a.get(
									isWitness ? 'witness_account' : 'committee_member_account'
								)
							)
					)
				);
	...
	onProxyChange(current_proxy_input) {
		...
		if (
			!proxyAccount ||
			(proxyAccount && proxyAccount.get('id') !== this.state.proxy_account_id)
		) {
			this.setState({
				proxy_account_id: '',
	...
	getBudgetObject() {
		...
		if (budgetObject) {
			...
			let currentID =
				idIndex +
				Math.floor((now - new Date(timestamp).getTime()) / 1000 / 60 / 60) -
				1;
			if (idIndex >= currentID) return;
			let newID = '2.13.' + Math.max(idIndex, currentID);
			let newIDInt = parseInt(newID.split('.')[2], 10);
			FetchChainObjects(ChainStore.getObject, [newID], undefined, {}).then(
				(res) => {
					let [lbo] = res;
					if (lbo === null) {
	...
	render() {
		...
		const hideLegacy = (
			...
			>
				<Tooltip title={counterpart.translate('tooltip.legacy_explanation')}>
					<Switch
							...
							<Tab title="explorer.witnesses.title">
								<div className={cnames('content-block')}>
									...
									<VotingAccountsList
										...
										supported={
											this.state[hasProxy ? 'proxy_witnesses' : 'witnesses']
										}
										withSelector={false}
							...
							<Tab title="explorer.committee_members.title">
								...
								<div className={cnames('content-block')}>
									<VotingAccountsList
										...
										supported={
											this.state[hasProxy ? 'proxy_committee' : 'committee']
										}
										withSelector={false}
