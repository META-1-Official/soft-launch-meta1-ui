...
import {ChainStore, ChainValidation} from 'meta1-vision-js';
		...
		if (asset) {
			...
			if (backing_backing && backing_backing !== '1.3.0') {
				...
				if (
					...
					asset.get('precision') !== parseInt(this.props.assetPrecision, 10)
				) {
					this.setState({
						error: counterpart.translate(
...
class AccountAssetCreate extends React.Component {
	...
	onChangeBitAssetOpts(value, e) {
		...
		switch (value) {
			...
			case 'maximum_force_settlement_volume':
				bitasset_opts[value] =
					parseFloat(e.target.value) * assetConstants.GRAPHENE_1_PERCENT;
				break;
			case 'minimum_feeds':
	...
	_onUpdateInput(value, e) {
		...
		switch (value) {
			...
			case 'max_market_fee':
				if (
					new big(inputValue).times(precision).gt(GRAPHENE_MAX_SHARE_SUPPLY)
				) {
	...
	render() {
		...
		for (let key in permissionBooleans) {
			if (permissionBooleans[key] && key !== 'charge_market_fee') {
				flags.push(
					getFlag(key, this._onFlagChange.bind(this, key), flagBooleans[key])
				);
			}
		}
						...
						>
							<Tab title="account.user_issued_assets.primary">
								<div className="small-12 grid-content">
									...
									{errors.symbol ? (
										<p className="grid-content has-error">{errors.symbol}</p>
									) : null}
