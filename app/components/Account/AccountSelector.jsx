...
class AccountSelector extends React.Component {
	...
	_fetchAccounts() {
		...
		if (search_array.length > 0) {
			...
			FetchChainObjects(ChainStore.getAccount, search_array, 3000, {}).then(
				(accounts) => {
		...
		if (!!props.onAccountChanged) {
			FetchChain('getAccount', accountName, undefined, {
				...
				.then((account) => {
					if (
						!!account &&
						((this.props.requireActiveSelect && inputType == 'select') ||
							!this.props.requireActiveSelect)
					) {
			...
			!this.props.label || !selectedAccount ? null : (
				...
				>
					<label
						className={cnames(
							'right-label',
							selectedAccount.isContact || selectedAccount.isOwnAccount
								? 'positive'
								: null,
		...
		return (
			...
			>
				<Form className="full-width" layout="vertical" style={this.props.style}>
					<Form.Item
						label={
							this.props.label ? counterpart.translate(this.props.label) : ''
						}
