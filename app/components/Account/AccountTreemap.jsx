...
class AccountTreemap extends React.Component {
	...
	render() {
		let {balanceObjects, core_asset, marketStats, preferredAsset, history} =
			this.props;
		...
		if (balanceObjects && balanceObjects.length > 0) {
			...
			accountBalances = balanceObjects.forEach((balance) => {
				if (!balance) return;
				let balanceObject =
					typeof balance == 'string' ? ChainStore.getObject(balance) : balance;
		...
		return (
			<AccountTreemapView accountBalances={accountBalances} config={config} />
		);
	}
}
