			...
			if (item) {
				...
				if (item.policy && item.policy[0] !== 2) {
					let start = Math.floor(
						new Date(item.policy[1].start_claim + 'Z').getTime() / 1000
					);
					...
					if (start > 0) {
						...
						let seconds_last_updated = Math.floor(
							new Date(
								item.policy[1].coin_seconds_earned_last_update + 'Z'
							).getTime() / 1000
						);
						let seconds_earned =
						...
						days_earned = utils.format_number(
							utils.get_asset_amount(seconds_earned / 86400, cvbAsset),
							0
						);
						...
						days_remaining = utils.format_number(
							(seconds_period * (1 - available_percentage)) / 86400 || 0,
							2
						);
					}
				} else {
			...
			},
			{
				...
				render: (item) => {
					return item.canClaim ? (
						<Button onClick={() => this.onClaim(item)} type="secondary">
