...
class WorkerList extends React.Component {
				...
				: {
						...
						render: (item) => {
							return (
								...
								>
									{item ? item : counterpart.translate('account.votes.expired')}
								</span>
							);
						},
				  },
					...
					return (
						<span>
							...
							>
								...
								>
									<Icon name="share" size="2x" title="icons.share" />
								</a>
							</div>
						...
						render: (item) => {
							return (
								...
								>
									{item.isExpired
										? '-'
										: utils.format_number(item.fundedPercent, 2) + '%'}
								</span>
							);
						},
				  },
					...
					return (
						<span
							...
							onClick={
								item.proxy
									? () => {}
									: this[item.approvalState ? 'onReject' : 'onApprove'].bind(
											this,
											item
									  )
							}
					...
					return (
						...
						>
							{!item.proxy ? (
								<Icon
									name={
										item.approvalState ? 'checkmark-circle' : 'minus-circle'
									}
									title={
										item.approvalState
	...
	getData(workers, voteThreshold = 0) {
		...
		return workers.map((item, index) => {
			...
			let endDate = counterpart.localize(new Date(worker.work_end_date + 'Z'), {
				type: 'date',
				format: 'short_custom',
			});
	...
	_getMappedWorkers(workers, maxDailyPayout, filterSearch) {
		...
		let mapped = workers
			...
			.map((worker) => {
				...
				worker.votes =
					worker.get('total_votes_for') - worker.get('total_votes_against');
	...
	render() {
		...
		const hideProposals = (filteredWorker, compareWith) => {
			...
			const manualHidden = hiddenProposals.includes(filteredWorker.get('id'));
