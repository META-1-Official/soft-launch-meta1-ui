...
class Proposals extends Component {
	...
	_setProposals() {
		...
		if (account.get('proposals').size) {
			...
			proposals.forEach((proposal) => {
				...
				proposal.requiredPermissions = permission_utils.unnest(required, type);
			});
	...
	render() {
		...
		if (
			(account.get('proposals').size > 0 && this._proposals.length == 0) ||
		...
		let proposalRows = this._proposals.reduce((result, proposal, index) => {
			...
			if (keys.length) {
				keys.forEach((key) => {
					...
					if (
						isMine &&
						!proposal.proposal.get('available_key_approvals').includes(key)
					) {
						keyNames.push(key);
					}
				});
				...
				<tr className="top-left-align" key={`${proposalId}_content`}>
					...
					<td>
						{requiredPermissions.map((account, index) => (
							...
							>
								<LinkToAccountById subpage="permissions" account={account.id} />
							</div>
						))}
					</td>
					<td>
					...
					<td className="approval-buttons">
						{isScam && (
							<Tooltip title={counterpart.translate('tooltip.propose_scam')}>
								<div className="tooltip has-error scam-error">SCAM ATTEMPT</div>
							</Tooltip>
						)}
						{this.props.hideFishingProposals && !isScam && isUnknown && (
							<Tooltip title={counterpart.translate('tooltip.propose_unknown')}>
								<div className="tooltip has-error scam-error">
									UNKNOWN SOURCE
								</div>
							</Tooltip>
						)}
						{!isScam && (!isUnknown || !this.props.hideFishingProposals) && (
							<button
								onClick={
									canApprove
								...
								className={'button primary' + (canApprove ? '' : ' hidden')}
							>
								<span>
		...
		return (
			<div>
				<table className={'table proposals compact ' + this.props.className}>
					<thead>
						<tr>
							<th>
