...
const stateSetter =
	(that, key, transform = (value) => value) =>
	(value) =>
...
class FeePoolOperation extends React.Component {
	...
	onClaimPool = () =>
		AssetActions.claimPool(this.props.asset, this.state.claimPoolAmountAsset);
	...
	renderClaimPool() {
		...
		const balanceText = !!dynamicObject ? (
			...
			>
				<Translate component="span" content="transfer.available" />
				:&nbsp;
				<FormattedAsset amount={dynamicObject.get('fee_pool')} asset={coreID} />
			</span>
		) : null;
		...
		return (
			<div>
				...
				<div style={{paddingTop: '1rem'}} className="button-group">
					...
					<button className="button outline" onClick={this.reset.bind(this)}>
						<Translate content="account.perm.reset" />
					</button>
				</div>
			</div>
