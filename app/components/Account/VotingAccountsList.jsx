...
const accountItemRow = (props) => {
	...
	let link =
		url && url.length > 0 && url.indexOf('http') === -1 ? 'http://' + url : url;
...
class VotingAccountsList extends React.Component {
				...
				render: (item) => {
					return (
						<span
							...
							onClick={
								item.proxy ? () => {} : item.onAction.bind(this, item.item_id)
							}
						>
	...
	render() {
		...
		let item_rows = this.props.items
			...
			.map((i, idx) => {
				let action =
					this.props.supported && this.props.supported.includes(i.get('id'))
						? 'remove'
						: 'add';
				...
				return accountItemRow({
					...
					onAction:
						action === 'add' ? this.props.onAddItem : this.props.onRemoveItem,
