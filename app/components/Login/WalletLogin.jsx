		...
		return (
			...
			>
				<div className="tooltip-text" onClick={(e) => e.stopPropagation()}>
					...
					<span onClick={() => ReactTooltip.hide()} className="close-button">
						×
					</span>
				</div>
			</ReactTooltip>
	...
	renderUploadInputForSmall() {
		...
		return (
			<div>
				...
				>
					{isDownloaded ? (
						<span className="bin-name">{this.props.backup.name}</span>
					) : (
						<span>&nbsp;</span>
					)}
		...
		return (
			...
			>
				<div
					onDragOver={(e) => e.preventDefault()}
					onDrop={(e) => this.onDropBinFile(e)}
					className={`file-input-container ${isInvalid ? 'invalid' : ''} ${
						isDrop ? 'dropHover' : ''
				...
				>
					...
					{isDownloaded ? (
						...
						<img className="bin-file" src="bin-file/error.svg" alt="bin-file" />
					) : (
						<span>
							<img
					...
					<span className="upload-text text-left no-overflow">
						{isDownloaded ? (
							<p className="bin-name">
								{this.props.backup.name} ({this.props.backup.size} bytes)
							</p>
						) : isInvalid ? (
							<Translate
	...
	render() {
		return (
			<div onClick={this.props.onChangeActive} className="wallet-block">
				...
				{new FileReader().readAsBinaryString || !this.props.active ? null : (
					<Translate
						component="p"
						className="error"
