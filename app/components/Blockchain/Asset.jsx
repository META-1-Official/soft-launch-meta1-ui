...
class Asset extends React.Component {
		...
		if (this.props.asset.has('bitasset')) {
			...
			if (!!feedPrice) {
				...
				try {
					Apis.instance()
						.db_api()
						.exec('get_collateral_bids', [this.props.asset.get('id'), 100, 0])
						.then((coll_orders) => {
	...
	renderFlagIndicators(flags, names) {
		return (
			<div>
				{names.map((name) => {
					return (
						<AssetFlag key={`flag_${name}`} name={name} isSet={flags[name]} />
					);
				})}
			</div>
		);
	...
	renderSummary(asset) {
		...
		var currentSupply = dynamic ? (
			<tr>
				...
				<td>
					<FormattedAsset amount={dynamic.current_supply} asset={asset.id} />
				</td>
			</tr>
		) : null;
	...
	renderPriceFeed(asset) {
		...
		var feedPrice = this.formattedPrice(assetUtils.extractRawFeedPrice(asset));
				...
				>
					<tbody>
						...
						<tr>
							...
							<td>{bitAsset.options.feed_lifetime_sec / 60 / 60}</td>
						</tr>
						<tr>
							<td>
						...
						<tr>
							...
							<td>{currentFeed.maintenance_collateral_ratio / 1000}</td>
						</tr>
						...
						<tr>
							...
							<td>{currentFeed.maximum_short_squeeze_ratio / 1000}</td>
						</tr>
					</tbody>
				</table>
			</Panel>
	...
	renderSettlement(asset) {
		...
		if (isGlobalSettle) {
			...
			current_collateral_value =
				current_collateral_value /
				Math.pow(10, assets[bitAsset.options.short_backing_asset].precision);
			...
			if (feedPrice) {
				settlement_fund_collateral_ratio =
					current_collateral_value / feedPrice.toReal() / current_supply_value;
					...
					{isGlobalSettle ? (
						...
						<tbody>
							...
							<tr>
								...
								<td>
									<FormattedAsset asset={asset.id} amount={currentSettled} />
								</td>
							</tr>
							<tr>
								<td>
								...
								<td>
									{currentSettled == 0
										...
										: Math.round(
												100 -
													(currentSettled /
														(currentSupply * (maxSettlementVolume / 10000))) *
														100,
												2
										  )}
									%
	...
	renderAssetOwnerUpdate(asset) {
		return (
			<Panel
				header={<Translate content="account.user_issued_assets.update_owner" />}
			>
				<Translate
					component="p"
	...
	renderFeedPublish(asset) {
		return (
			<Panel
				header={<Translate content="transaction.trxTypes.asset_publish_feed" />}
			>
				<Translate component="p" content="explorer.asset.feed_producer_text" />
				<AssetPublishFeed
					asset={asset.id}
					account={this.props.currentAccount}
					currentOwner={asset.issuer}
	...
	renderCollateralBid(asset) {
		return (
			<Panel header={<Translate content="explorer.asset.collateral.bid" />}>
				<Translate
					component="p"
	...
	renderFeePoolFunding(asset) {
		return (
			<Panel header={<Translate content="explorer.asset.fee_pool.fund" />}>
				<Translate
					component="p"
	...
	renderFeePoolClaiming(asset) {
		...
		return (
			<Panel
				header={<Translate content="explorer.asset.fee_pool.claim_balance" />}
			>
				<FeePoolOperation
					asset={asset.symbol}
					funderAccountName={this.props.currentAccount}
	...
	renderFeesClaiming(asset) {
		...
		return (
			<Panel
				header={<Translate content="transaction.trxTypes.asset_claim_fees" />}
			>
				<FeePoolOperation
					asset={asset.symbol}
					dynamic={dynamic}
	...
	renderPermissions(asset) {
			...
			<tr>
				...
				<td>
					{asset.symbol === 'USD' ? (
						...
						<FormattedAsset amount={+options.max_market_fee} asset={asset.id} />
					)}
				</td>
			</tr>
		) : null;
			...
			<tr>
				...
				<td>
					{asset.symbol === 'USD' ? (
						...
						<FormattedAsset amount={+options.max_supply} asset={asset.id} />
					)}
				</td>
			</tr>
		);
		...
		var whiteLists = permissionBooleans['white_list'] ? (
			<div>
				...
				{!!options.blacklist_markets && !!options.blacklist_markets.length && (
					<React.Fragment>
						<br />
				...
				{!!options.whitelist_markets && !!options.whitelist_markets.length && (
					<React.Fragment>
						<br />
		...
		return (
			<Panel header={<Translate content="explorer.asset.permissions.title" />}>
				<div>
					<table
						className="table key-value-table table-hover"
						style={{padding: '1.2rem'}}
		...
		columns = [
			{
				...
				fixed: 'left',
				width: 150,
				title: <Translate content="explorer.asset.price_feed_data.publisher" />,
				dataIndex: 'publisher',
				sorter: (a, b) => {
			...
			},
			{
				...
				render: (item) => {
					...
					let median_offset = ((price / currentFeedPrice) * 100 - 100).toFixed(
						2
					);
					return (
						<React.Fragment>
			...
			},
			{
				...
				fixed: 'right',
				width: 150,
				title: <Translate content="explorer.asset.price_feed_data.published" />,
			...
			columns = [
				...
				},
				{
					...
					render: (item) => {
						...
						return (
							<React.Fragment>
								<div className={classNames}>{item.ratio.toFixed(3)}</div>
							</React.Fragment>
						);
					},
				},
	...
	_renderCollBidTable() {
		...
		columns = [
			...
			{
				key: 'debt_cum',
				title: <Translate content="transaction.cumulative_borrow_amount" />,
				dataIndex: 'debt_cum',
				render: (item) => {
					return (
						<FormattedAsset
			...
			{
				key: 'price',
				title: <Translate content="explorer.asset.collateral_bid.bid" />,
				dataIndex: 'price',
				render: (item) => {
					return (
						<FormattedPrice
	...
	renderFeedTables(asset) {
		...
		return (
			...
			>
				<Tabs.TabPane
					tab={counterpart.translate('explorer.asset.price_feed_data.title')}
					key="feed"
				>
			...
			<div className="grid-container asset-page">
				<div className="grid-block page-layout">
					<div className="grid-block main-content wrap">
						<div className="grid-block medium-up-1" style={{width: '100%'}}>
						...
						>
							...
							<Tabs.TabPane
								tab={counterpart.translate('explorer.asset.actions')}
								key="actions"
							>
								<Collapse className="asset-collapse">
