...
const getVotesName = (item) => {
	if (item.id.startsWith('1.14')) {
		...
		return (
			'Witness ' +
			(witness_account ? witness_account : ' account ' + item.witness_account)
		);
	} else {
...
export const AccountUpdate = ({op, fromComponent, collapsed}) => {
	...
	if (fromComponent === 'proposed_operation' && !collapsed) {
		...
		const _votes = _account.get('options').get('votes').toArray();
		...
		if (owner) {
			...
			if (
				_account.get('owner').get('weight_threshold') !== owner.weight_threshold
			) {
