...
class Preimage extends React.Component {
	...
	generateRandom(e = {target: {}}) {
		...
		e.target.value = key.get_random_key().toWif().substr(10, 30);
	...
	render() {
		...
		let hashMatch =
			...
			this.state.preimage_hash_calculated !== null
				? this.state.preimage_hash_calculated == this.props.preimage_hash
				: null;

		return (
			<Form.Item label={label}>
				<span>
					{counterpart.translate('showcases.htlc.preimage_has_been_created')}
				</span>
				<Input.Group className="content-block transfer-input preimage-row">
					...
					>
						<Input
							style={{
								width: '60%',
								color:
									hashMatch == null ? undefined : hashMatch ? 'green' : 'red',
							}}
							name="preimage"
							id="preimage"
							type="text"
					...
					<Tooltip
						title={counterpart.translate('showcases.htlc.tooltip.new_random')}
						mouseEnterDelay={0.5}
					>
						<Button
							type="primary"
				...
				<Input.Group className="content-block transfer-input preimage-row">
					...
					>
						<Input
							...
							placeholder={counterpart.translate('showcases.htlc.hash')}
					...
					>
						<Input
							...
							placeholder={counterpart.translate('showcases.htlc.size')}
							disabled={this.state.stage == 1}
						/>
					</Tooltip>
					<div style={{float: 'right'}}>
...
class HtlcModal extends React.Component {
	...
	getInitialState() {
		...
		return {
			...
			claim_period: 86400,
			period: 'one_day',
			expirationDate: moment().add('seconds', 180).add(1, 'day'),
		};
	}
	...
	_checkBalance() {
		...
		if (!feeBalanceObject || feeBalanceObject.get('balance') === 0) {
			this.setState({fee_asset_id: this.state.fee_asset_id}, this._updateFee);
		}
	...
	onPreimageChanged({preimage, preimage_cipher, preimage_hash, preimage_size}) {
		let stateChange = {};
		...
		const amountHeader = (
			<div className="form-input-header--label">
				...
				<div className="form-input-header--right">
					...
					>
						{counterpart.translate('showcases.htlc.expiration_period.one_day')}
					</span>
					<span
						className={cnames('period-row', {
					...
					>
						{counterpart.translate('showcases.htlc.expiration_period.two_days')}
					</span>
					<span
						className={cnames('period-row', {
					...
					>
						{counterpart.translate('showcases.htlc.expiration_period.one_week')}
					</span>
				</div>
			</div>
		);
			...
			>
				<div className="grid-block vertical no-overflow">
					<Form className="full-width" layout="vertical">
						...
						{isExtend ? (
							<Form.Item
								label={counterpart.translate('showcases.htlc.preimage')}
							>
								<Input
									type="text"
									value={preimage_hash || ''}
