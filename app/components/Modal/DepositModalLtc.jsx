	...
	_copy(e) {
		try {
			if (this.props.depositAddress)
				e.clipboardData.setData('text/plain', this.props.depositAddress);
			else
	...
	render() {
		return (
			<div>
				...
				<h5>Minimum deposit: 0.01 LTC</h5>
				...
				<h6>
					<b>IMPORTANT:</b> Send only LTC to this deposit address. Sending less
					than 0.01 LTC or any other currency to this address may result in the
...
export default class DepositModal extends React.Component {
	...
	getDepositAddress() {
		fetch('https://gateway.dev.meta-exchange.vision/api-gateways/ltc')
			.then((response) => {
				fetch('https://gateway.dev.meta-exchange.vision/api/wallet/init/ltc', {
					method: 'POST',
					headers: {
