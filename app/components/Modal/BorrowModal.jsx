...
class BorrowModalContent extends React.Component {
	...
	componentWillReceiveProps(nextProps) {
		...
		if (
			...
			nextProps.quoteAssetObj.get('id') !== this.props.quoteAssetObj.get('id')
		) {
	...
	_maximizeDebt() {
		...
		let maximumCollateral =
			...
			10;
		const debtAmount =
			(maximumCollateral / this.state.collateral_ratio) * this._getFeedPrice();
	...
	_validateFields(newState) {
		...
		if (isOriginalBelowMCR && newState.debtAmount > original_position.debt) {
			...
			errors.close_maintenance = counterpart.translate('borrow.errors.close', {
				mr: maintenanceRatio,
			});
		}
	...
	render() {
		let {quoteAssetObj, backingAssetObj, debtBalanceObj, collateralBalanceObj} =
			this.props;
		let {
			debtAmount,
			collateral,
		...
		if (
			...
			collateral_ratio =
				...
				'.' +
				collateral_ratio.toString().split('.')[1].substr(0, 2);
		}
		...
		if (
			...
			target_collateral_ratio =
				...
				'.' +
				target_collateral_ratio.toString().split('.')[1].substr(0, 2);
		}
		...
		return (
			...
			>
				<BorrowModalView
					...
					isValid={
						isPredictionMarket || (!isPredictionMarket && !isNaN(feed_price))
					}
