...
class DirectDebitClaimModal extends React.Component {
	...
	checkLimit() {
		const {withdrawal_limit, amount, limitError, asset, claimedAmount} =
			this.state;
	...
	isPayerBalanceWarning() {
		...
		const limitAmount = utils.get_asset_amount(withdrawal_limit.amount, asset);

		const balanceID = to_account.getIn(['balances', withdrawal_limit.asset_id]);
		if (!balanceID) {
		...
		if (payerBalance < limitAmount - claimedAmount && !payerBalanceWarning) {
			this.setState({payerBalanceWarning: true});
		} else if (
				...
				balance = (
					<span>
						...
						<span
							...
							onClick={this.setTotalLimit(withdrawal_limit.amount)}
						>
							<LimitToWithdraw
						...
						<div>
							...
							<div className="content-block">
								<AccountSelector
									...
									accountName={!!from_account ? from_account.get('name') : ''}
								...
								value={
									current_period_expires_date
										? counterpart.localize(
												new Date(current_period_expires_date),
												{
													type: 'date',
													format: 'full',
												}
...
export default connect(DirectDebitClaimModal, {
	listenTo() {
