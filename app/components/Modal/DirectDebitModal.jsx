		...
		if (
			...
			if (toAccount && toAccount.get) {
				const timeStart = moment
					...
					.valueOf();
				const timeEnd = moment.utc(operation.payload.expiration).valueOf();
				...
				for (let i = 0; i < periodTypes.length; i++) {
					if (
						operation.payload.withdrawal_period_sec >= periodTypes[i].seconds
					) {
						...
						periodAmount = Math.round(
							operation.payload.withdrawal_period_sec / currentPeriod.seconds
						);
						break;
					}
				}
				...
				this.setState({
					...
					period_start_time: moment.utc(operation.payload.period_start_time),
				});
			}
		}
	}
			...
			<Modal
				title={
					operation && operation.type === 'create'
						? counterpart.translate('showcases.direct_debit.create_new_mandate')
						: counterpart.translate('showcases.direct_debit.update_mandate')
				}
				...
				<div className="grid-block vertical no-overflow">
					<form noValidate>
						...
						<div className="content-block transfer-input">
							{/*  START DATE  */}
							<label className="left-label">
								{counterpart.translate('showcases.direct_debit.start_date')}
							</label>
							<Tooltip
								title={counterpart.translate(
							...
							>
								<DatePicker
									...
									disabledDate={(current) =>
										current && current < moment().add(2, 'minutes')
									}
								/>
							</Tooltip>
						</div>
