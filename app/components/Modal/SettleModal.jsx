...
import {Alert, Form, Modal, Button, Tooltip} from 'antd';
						...
						<span>
							<TranslateWithLinks
								...
								keys={[
									...
									{
										value: <AssetName name={asset.get('symbol')} />,
										arg: 'long',
									},
								]}
							/>
...
class ModalContent extends React.Component {
	...
	render() {
		...
		const footer = [
			<Tooltip
				title={
					isFundsToLow ? counterpart.translate('tooltip.lack_funds') : null
				}
			>
				<Button
					key={'submit'}
		...
		const estimatedDelay = !isGlobalSettled
			? (settlementDelay +
					Math.floor(amount / maxSettlementVolume) * maintenanceInterval) /
			  3600
			: 0;
		...
		let modalContent = isPredictionMarket ? (
			<Alert
				message={counterpart.translate('tooltip.settle_market_prediction')}
				type="info"
				showIcon
			/>
		) : (
