...
class MarketsTable extends React.Component {
	...
	getTableData(row) {
		...
		return {
			key: marketID,
			star: (
				<div onClick={this._toggleFavoriteMarket.bind(this, quote, base)}>
					<Icon
			...
			hour_24:
				!marketStats || !marketStats.change || marketStats.change === '0.00'
					? 0
					: marketStats.change,
			volume:
		...
		const marketRows = markets
			...
			.map((row) => {
				...
				if (row.isHidden !== this.state.showHidden) {
					...
					const {isBitAsset: quoteIsBitAsset} = utils.replaceName(quoteObject);
					const {isBitAsset: baseIsBitAsset} = utils.replaceName(baseObject);
...
export default connect(MarketsTable, {
	...
	getProps() {
		...
		return {
			...
			onlyLiquid: SettingsStore.getState().viewSettings.get('onlyLiquid', true),
		};
	},
});
