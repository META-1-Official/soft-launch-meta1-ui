...
class NewsHeadline extends React.Component {
	...
	shouldComponentUpdate(props, state) {
		return (
			Object.keys(this.state.news).length !== Object.keys(state.news).length ||
	...
	async getNewsThroughAsset() {
		try {
			...
			const news = filterNews(notificationList, this.props.hiddenNewsHeadline);
			this.setState({news});
		} catch (err) {
	...
	onClose(item) {
		...
		if (this.props.hiddenNewsHeadline.indexOf(getNewsItemHash(item)) == -1) {
