...
class ImportKeys extends Component {
	...
	/** META1 1.0 client wallet_export_keys format. */
	_parseImportKeyUpload(json_contents, file_name, update_state) {
		...
		// META1 1.0 wallets may have a lot of generated but unused keys or spent TITAN addresses making
					...
					genesis_filter.filter(account_keys, (status) => {
						...
						if (!filter_status.length)
							...
							let last_account_name =
								filter_status[filter_status.length - 1].account_name;
	...
	/**
    META1 1.0 hosted wallet backup (wallet.bitshares.org) is supported.

    META1 1.0 native wallets should use wallet_export_keys instead of a wallet backup.
    ...
    */
	_parseWalletJson(json_contents) {
		...
		let savePubkeyAccount = function (pubkey, account_name) {
			...
			address = 'META1' + address.substring(3);
				...
				try {
					...
					if (!public_key_string) {
						...
						if (!same_prefix_regex.test(public_key_string))
							...
							public_key_string =
								ChainConfig.address_prefix + public_key_string.substring(3);
					}
					...
					notification.error({
						message: counterpart.translate('notifications.import_keys_error', {
		...
		WalletDb.importKeysWorker(private_key_objs)
			.then((result) => {
				...
				notification.success({
					message: counterpart.translate('wallet.import_key_success', {
						count: import_count,
					}),
				});
	...
	addByPattern(contents) {
		if (!contents) {
			this.setState({
				errorTextMessage: counterpart.translate('wallet.wif_import_error'),
			});
			return false;
		}
		if (contents.length !== WIF_KEY_LENGTH) {
			this.setState({
				errorTextMessage: counterpart.translate('wallet.wif_length_error'),
			});
			return false;
		}
		let count = 0,
					...
					account_rows.push(
						<tr key={status.account_name}>
							...
							<td>
								{filtering ? (
									<span>
										Filtering {Math.round((status.count / status.total) * 100)}{' '}
										%{' '}
									</span>
								) : (
								...
								{privateKey ? (
									<form onSubmit={this.onWif.bind(this)}>
										...
										<div className="importError">
											<span className="red">{this.state.errorTextMessage}</span>
										</div>
										<Button
											type="primary"
											htmlType="submit"
