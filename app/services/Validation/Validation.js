...
export const Messages = {
	...
	Type: (type = '', name = '') => {
		if (name)
			return counterpart.translate(`validation.messages.types.${type}Named`, {
				name: name,
				type: type,
			});
...
export const Rules = {
	...
	type: function (props) {
		...
		if (type === '')
			throw new Error('[Validation] Rules.Type the property type is missed');

		if (!typesCheck[type])
			throw new Error(
				`[Validation] Rules.Type the property type '${
					props && props.type
	...
	range: function (props = {}) {
		...
		if (max === undefined || isNaN(max))
			throw new Error(
				`[Validation] Rules.Range the property max '${
					props && props.max
		...
		if (max < min)
			throw new Error(
				`[Validation] Rules.Range the property min '${
					props && props.min
				}' cannot be higher than max '${props && props.max}'`
			);
	...
	min: function (props) {
		...
		if (min === undefined || isNaN(min))
			throw new Error(
				`[Validation] Rules.Min the property min '${
					props && props.min
				}' is incorrect. Should be a number`
			);
	...
	max: function (props) {
		...
		if (max === undefined || isNaN(max))
			throw new Error(
				`[Validation] Rules.Min the property max '${
					props && props.max
				}' is incorrect. Should be a number`
			);
