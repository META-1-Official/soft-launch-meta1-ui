		...
		return new Promise((resolve, reject) => {
			...
			function done_pinging() {
				...
				if (
					...
					thiz._connectionManager.url = thiz._connectionManager.urls[0];
					console.log(
						'auto selecting to ' +
						...
						return Promise.all([
							...
							WalletDb.loadDbData()
								.then(() => {
									if (chainChanged) {
										AccountStore.reset();
										return AccountStore.loadDbData(currentChain).catch(
											(err) => {
												console.error(err);
											}
										);
...
class Pinger {
	...
	_notifyCallback() {
		if (this._callbackWasCalled) {
			...
			return this._suitableNodeFound || this._current == this._nodeURLs.length;
		}
	}
...
class PingStrategy {
	...
	getFromRegion(regionKey, countryKey = null, amount = 10, randomOrder = true) {
		let filtered = [];
