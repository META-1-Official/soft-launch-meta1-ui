...
/**
    This should only be applied to a META1 1.0 export file taken on the
    discontinued chain. Any public key string or address (all 5 formats) carried
    over to the META1 2.0 genesis block will be in this filter.
...
export default class GenesisFilter {
	...
	inGenesis(pubkey_or_address) {
		...
		for (var hashes = 0; hashes < 3; hashes++) {
			...
			var bit_address =
				parseInt(hex.slice(-3).toString('hex'), 16) % this.bits_in_filter; // 3090564
	...
	filter(account_keys, status) {
		if (!this.isAvailable()) {
			console.log('WARN: Missing bloom filter for META1 0.9.x wallets');
