...
const ADDRESS_PREFIX =
	process.env.CURRENT_NET === 'MAIN_NET'
		? process.env.MAIN_NET_PREFIX
		: process.env.TEST_NET_PREFIX;
		...
		Promise.all([
			...
			.then((result) => {
				...
				Promise.all([
					...
					if (feeBalance && feeBalance.get('balance') >= fee.getAmount())
						hasBalance = true;
		...
		for (let option of options) {
			...
			if (option === 'price_per_kbyte') {
				if (data.type === 'memo' && !!data.content) {
					...
					let memoFromKey =
						ADDRESS_PREFIX +
						'6B1taKXkDojuC1qECjvC7g186d8AdeGtz8wnqWAsoRGC6RY8Rp';

					// Memos are optional, but if you have one you need to encrypt it
					let memoToKey =
						ADDRESS_PREFIX +
						'8eLeqSZZtB1YHdw7KjQxRSRmaKAseCxhUSqaLxUdqvdGpp6nck';
