...
const MarketUtils = {
	...
	limitByPrecision(value, asset, floor = true) {
		...
		if (
			...
			(splitString.length === 2 && splitString[1].length <= assetPrecision)
		) {
			return value;
		}
	...
	getFinalPrice(
		...
		const {marketName: fromMarket} = this.getMarketName(fromAsset, coreAsset);
		const {marketName: directMarket} = this.getMarketName(fromAsset, toAsset);
		...
		cacheClearTimer();
		if (real)
			return finalPrice.toReal(toAsset.get('id') !== finalPrice.base.asset_id);
		return finalPrice;
	},
	...
	parseOrder(order, base, quote, invert = false) {
		...
		if (order.debt) {
			if (invert) {
				// Price in USDT/META1, amount should be in META1, value should be in USDT, debt is in USDT
				// buy is in USDT, sell is in META1
				// quote is USDT, base is META1
				value = order.debt / quotePrecision;
				amount = this.limitByPrecision(value / price.full, base);
			} else {
				// Price in META1/USDT, amount should be in USDT, value should be in META1, debt is in USDT
				// buy is in USDT, sell is in META1
				// quote is USDT, base is META1
