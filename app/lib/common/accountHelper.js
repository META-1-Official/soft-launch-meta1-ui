			...
			.then((call_orders) => {
				...
				FetchChain('getAsset', assets, 6000).then((assets) => {
					...
					call_orders.forEach((co) => {
						let debtAsset =
							assetsMap[co.getIn(['call_price', 'quote', 'asset_id'])];
						let collateralAsset =
							assetsMap[co.getIn(['call_price', 'base', 'asset_id'])];
						...
						if (sp.base.asset_id === sp.quote.asset_id) {
							...
							let mr =
								debtAsset.bitasset.current_feed.maintenance_collateral_ratio /
								1000;
							let price = new FeedPrice({
								priceObject: asset_utils.extractRawFeedPrice(debtAsset),
								market_base:
									asset_utils.extractRawFeedPrice(debtAsset).quote.asset_id,
								sqr: debtAsset.bitasset.current_feed
							...
							let current = {
								ratio:
									...
									(debt.getAmount({real: true}) / price.toReal()),
							};
