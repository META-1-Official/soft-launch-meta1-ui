...
class Asset {
	...
	equals(asset) {
		return (
			this.asset_id === asset.asset_id && this.getAmount() === asset.getAmount()
		);
	}
...
class LimitOrder {
	constructor(order, assets, market_base) {
		...
		this.expiration =
			order.expiration && new Date(utils.makeISODateString(order.expiration));
	...
	ne(order) {
		return (
			this.sell_price.ne(order.sell_price) || this.for_sale !== order.for_sale
		);
	}
	...
	_getMaxCollateralToSell() {
        ...
        */
		if (
			...
			let nominator =
				this.debt * this.target_collateral_ratio - this.collateral * feed_price;
			let denominator = this.target_collateral_ratio * match_price - feed_price;
	...
	amountToReceive() {
		...
		to_receive.setAmount({
			sats:
				...
				((GRAPHENE_100_PERCENT - this.offset_percent) / GRAPHENE_100_PERCENT),
		});
