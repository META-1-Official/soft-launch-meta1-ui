...
const ApplicationApi = {
	...
	_get_memo_keys(account, with_private_keys = true) {
		...
		if (with_private_keys) {
			...
			if (!memo.private_key) {
				...
				throw new Error(
					'Missing private memo key for sender: ' + account.get('name')
				);
			}
		}
		return memo;
	...
	createWorker(options, account) {
		return new Promise((resolve, reject) => {
			...
			if (!owner)
				reject(new Error("Can't find the owner account, please try again"));
	...
	async createVestingBalance(
		...
		let transactionBuilder = new TransactionBuilder();
		let op = transactionBuilder.get_type_operation('vesting_balance_create', {
			fee: {
				amount: 0,
				asset_id: objects.feeAsset.get('id'),
			},
